...
$query_itens_pedido = $db_prod->query("SELECT i.* from TMPPEDIDOS p join TMPITENS_PEDIDO i on p.ID_PEDIDO = i.ID_PEDIDO  where p.TRANSMITIDO = 'N'");
    ...
    $result_sql = $sql->fetchAll(PDO::FETCH_ASSOC);
        foreach($result_sql as $row=> $mauricio){
            ...
            if($s == 0){
                ...
                $insere_banco = $db_prod->query("SELECT p.* from TMPPEDIDOS p join TMPITENS_PEDIDO i on p.ID_PEDIDO = i.ID_PEDIDO where p.TRANSMITIDO = 'N' and p.ID_PEDIDO = {$teste['ID_PEDIDO']}  group by p.ID_PEDIDO");
  ...
  $resultado_validacao_itens = $valida_se_item_existe_banco->fetchAll(PDO::FETCH_ASSOC);
    ...
    foreach($resultado_validacao_itens as $row=> $itens_pesquisados){
      ...
      if($itens_pesquisados['totals'] == 0){
        ...
        $itens_banco_caixa = $db_prod->query("SELECT i.* from TMPPEDIDOS p join TMPITENS_PEDIDO i on p.ID_PEDIDO = i.ID_PEDIDO  where p.TRANSMITIDO = 'N' and i.id = {$itens['ID']} ");
        ...
        foreach($result_consulta_itens_banco_caixa as $row=> $itens){
          ...
          $sql_item = "INSERT INTO TMPITENS_PEDIDO (
            ...
            if($pdo->exec($sql_item) == true){
              header('Location: gera_update_capa.php');
            }
