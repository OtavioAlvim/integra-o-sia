...
if (strpos($valor,"*") !== false) {
    ...
    if($total == 0){
        ...
        $inclui_produto = $pdo->query("SELECT produtos.ID_PRODUTO,produtos.CODBARRA,produtos.DESCRICAO,produtos.UNIDADE,produtos.UNITARIO FROM produtos where produtos.ID_PRODUTO = '{$cod_processado}' and produtos.ID_EMPRESA = 1");
    ...
    if($total == 0){
        ...
        //"encontramos $total registro no banco de dados";
        $inclui_produto = $pdo->query("SELECT produtos.ID_PRODUTO,produtos.CODBARRA,produtos.DESCRICAO,produtos.UNIDADE,produtos.UNITARIO FROM produtos where produtos.ID_PRODUTO = '{$codigo_prod_completo}' and produtos.ID_EMPRESA = 1");
        $executei = $inclui_produto->fetchAll(PDO::FETCH_ASSOC);
        foreach($executei as $row => $produtos){
...
if (strpos($valor,"*") !== false) {
    ...
    if($total == 0){
        ...
        //"encontramos $total registro no banco de dados";
        $inclui_produto = $pdo->query("SELECT produtos.ID_PRODUTO,produtos.CODBARRA,produtos.DESCRICAO,produtos.UNIDADE,produtos.UNITARIO FROM produtos where produtos.ID_PRODUTO = '{$coditem}' and produtos.ID_EMPRESA = 1");
        $executei = $inclui_produto->fetchAll(PDO::FETCH_ASSOC);
        foreach($executei as $row => $produtos){
            $id_produto = $produtos['ID_PRODUTO'];
            $codbarra = $produtos['CODBARRA'];
    ...
    if ($valida_balanca == 2) {
        ...
        if ($total == 0) {
            ...
            // "encontramos $total registro no banco de dados";
            $inclui_produto = $pdo->query("SELECT produtos.ID_PRODUTO,produtos.CODBARRA,produtos.DESCRICAO,produtos.UNIDADE,produtos.UNITARIO FROM produtos where produtos.ID_PRODUTO = '{$coditemm}' and produtos.ID_EMPRESA = 1");//falta validar esse campo
            $executei = $inclui_produto->fetchAll(PDO::FETCH_ASSOC);
            foreach($executei as $row => $produtos){
                $id_produto = $produtos['ID_PRODUTO'];
                $codbarra = $produtos['CODBARRA'];
