...
// $entregas = $conn->query("SELECT SUBSTR(e.nome,1,15) AS nomes,SUBSTR(e.endereco,1,30) AS enderecos,e.* FROM entregas e WHERE e.entregue = 'N' and e.cnpj = '{$_SESSION['cnpj']}' ORDER BY e.numero_pedido asc");
$entregas = $conn->query("SELECT * FROM entregas e WHERE e.entregue = 'N' and e.cnpj = '{$_SESSION['cnpj']}' ORDER BY e.numero_pedido asc");
...
<body>
            ...
            <div class="offcanvas offcanvas-end text-bg-dark" tabindex="-1" id="offcanvasDarkNavbar" aria-labelledby="offcanvasDarkNavbarLabel">
                ...
                <div class="offcanvas-body">
                    <ul class="navbar-nav justify-content-end flex-grow-1 pe-3">
                        <li class="nav-item">
                            <a class="nav-link" href="./entregue.php">Entregue</a>
                        </li>
                    </ul>
                </div>
            </div>
    ...
    <div class="container" style="margin-top: 100px; ">
        ...
        <h1>Pedidos aguardando entrega</h1>
        <hr>
        <!-- <div class="row row-cols-1 row-cols-sm-3 row-cols-md-4 g-4 " style="margin-top: 20px; ">



            <?php
            if (empty($resultado_entrega)) {
                echo "<h1>Aguardadndo pedidos para entrega</h1>";
            }
                ...
                <div class="col">
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <p class="card-title"><strong><?php echo $entrega['nome'] ?></strong> </p>
                            <p class="card-text"> <?php echo $entrega['endereco'] ?></p>
                            <div class="d-grid gap-2 d-md-block">
                                <form action="../processamento/baixa_entrega.php" method="post">
                                    <input type="hidden" name="id_entrega" value="<?php echo $entrega['numero_pedido'] ?>">
                                    <button class="btn btn-outline-success" type="submit">Entregue</button>
                                </form>
                            </div>
                        </div>
    ...
    <div class="container">
        <div class="row row-cols-1 row-cols-md-4 g-4" style="margin-top: 20px; ">
            <?php
            if (empty($resultado_entrega)) {
                echo "<h1>Aguardando pedidos para entrega</h1>";
            }
            foreach ($resultado_entrega as $linhas => $entrega) { ?>
                <div class="col">
                    <div class="card h-100">
                        ...
                        <div class="card-body">
                            <h5 class="card-title">Pedido n <?php echo $entrega['numero_pedido'] ?></h5>
                            <p class="card-text"><?php echo $entrega['nome'] ?></p>
                            <p> <?php echo $entrega['endereco'] ?></p>
                        </div>
                        <div class="card-footer">
                            <form action="../processamento/baixa_entrega.php" method="post">
                                <input type="hidden" name="id_entrega" value="<?php echo $entrega['numero_pedido'] ?>">
                                <div class="d-grid gap-2">
                                    <button type="submit" class="btn btn-outline-success">Entregue</button>
                                </div>
                            </form>
                        </div>
                    </div>
...
<!--MODAL ENTREGA FINALIZADA COM SUCESSO-->

<?php
if (isset($_SESSION['pedidoEntregue'])) :
?>
